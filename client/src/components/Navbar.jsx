...
const css = `
  ...
  .nb-nav {
    position: fixed; top: 0; z-index: 100; height: 64px; width: 100%;
  ...
  .nb-logo {
    text-decoration: none; display: flex; flex-direction: column; line-height: 1; flex-shrink: 0;
  }
  .nb-logo-main {
    ...
    animation: logoShimmer 4s ease infinite; letter-spacing: .02em;
  }
  ...
  .nb-logo-sub {
    ...
    color: rgba(201,168,76,.5); font-weight: 500; margin-top: 1px;
  }
  ...
  .nb-link {
    ...
    font-weight: 500; color: rgba(240,235,224,.5); padding: 6px 12px; border-radius: 2px;
    position: relative; transition: color .25s;
  }
  .nb-link::after {
    content: ''; position: absolute; bottom: 0; left: 12px; right: 12px; height: 1px;
    background: #c9a84c; transform: scaleX(0);
    transition: transform .3s cubic-bezier(.16,1,.3,1); transform-origin: left;
  }
  ...
  .nb-link.admin-link {
    color: rgba(201,168,76,.7); border: 1px solid rgba(201,168,76,.2); background: rgba(201,168,76,.06);
  }
  .nb-link.admin-link:hover {
    color: #c9a84c; border-color: rgba(201,168,76,.45); background: rgba(201,168,76,.1);
  }
  .nb-link.admin-link::after { display: none; }

  .nb-divider { width: 1px; height: 18px; background: rgba(201,168,76,.18); flex-shrink: 0; margin: 0 4px; }

  .nb-right { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }

  .nb-greeting {
    font-size: .7rem; letter-spacing: .1em; color: rgba(240,235,224,.35); font-weight: 300; white-space: nowrap;
  }
  .nb-greeting strong { color: rgba(201,168,76,.75); font-weight: 500; }

  .nb-cart-btn {
    position: relative; background: none; border: 1px solid rgba(201,168,76,.2); border-radius: 2px;
    width: 38px; height: 38px; display: flex; align-items: center; justify-content: center;
    color: rgba(240,235,224,.55); cursor: pointer;
    transition: border-color .25s, color .25s, background .25s;
  }
  .nb-cart-btn:hover { border-color: rgba(201,168,76,.55); color: #c9a84c; background: rgba(201,168,76,.06); }
  .nb-cart-count {
    position: absolute; top: -6px; right: -6px; background: #c9a84c; color: #080810;
    font-size: .52rem; font-weight: 800; width: 16px; height: 16px; border-radius: 50%;
  ...
  .nb-logout-btn {
    background: none; border: 1px solid rgba(239,68,68,.25); color: rgba(239,68,68,.6);
    font-family: 'DM Sans', sans-serif; font-size: .68rem; letter-spacing: .14em;
    text-transform: uppercase; font-weight: 600; padding: 7px 14px; border-radius: 2px;
    cursor: pointer; transition: background .25s, border-color .25s, color .25s; white-space: nowrap;
  }
  .nb-logout-btn:hover { background: rgba(239,68,68,.1); border-color: rgba(239,68,68,.5); color: #f87171; }

  .nb-register-btn {
    text-decoration: none; background: linear-gradient(135deg, #c9a84c, #e8d08a); color: #080810;
    font-size: .68rem; letter-spacing: .14em; text-transform: uppercase; font-weight: 700;
    padding: 8px 18px; border-radius: 2px; transition: transform .25s, box-shadow .25s;
    white-space: nowrap; position: relative; overflow: hidden;
  }
  .nb-register-btn::after {
    content: ''; position: absolute; inset: 0; background: rgba(255,255,255,.18);
    transform: translateX(-110%) skewX(-15deg); transition: transform .4s cubic-bezier(.16,1,.3,1);
  }
  .nb-register-btn:hover::after { transform: translateX(110%) skewX(-15deg); }
  .nb-register-btn:hover { transform: translateY(-1px); box-shadow: 0 8px 20px rgba(201,168,76,.35); }

  .nb-mobile-toggle {
    display: none; background: none; border: 1px solid rgba(201,168,76,.2); border-radius: 2px;
    width: 38px; height: 38px; align-items: center; justify-content: center;
    color: rgba(240,235,224,.6); cursor: pointer; flex-shrink: 0;
  }

  .nb-mobile-menu {
    display: none; flex-direction: column; background: rgba(8,8,16,.97);
    border-bottom: 1px solid rgba(201,168,76,.15); padding: 16px 28px 20px; gap: 2px;
    backdrop-filter: blur(16px);
  }
  .nb-mobile-menu.open { display: flex; }
  .nb-mobile-link {
    text-decoration: none; font-size: .75rem; letter-spacing: .14em; text-transform: uppercase;
    font-weight: 500; color: rgba(240,235,224,.5); padding: 11px 0;
  ...
  @media (max-width: 480px) { .nb-nav { padding: 0 16px; } }
`;
        ...
        <nav className={`nb-nav ${scrolled ? 'scrolled' : 'top'}`}>
          <div className="nb-inner">
            ...
            <ul className="nb-links">
              ...
              {user?.role === 'admin' && (
                <>
                  ...
                  <li><Link to="/admin" className="nb-link admin-link">Admin Panel</Link></li>
                </>
              )}
            </ul>
            ...
            <div className="nb-right">
              ...
              {/* ✅ FIXED: use setCartOpen not setOpen */}
